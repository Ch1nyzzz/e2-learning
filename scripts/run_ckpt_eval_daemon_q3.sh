...
ckpt_root=outputs/alfworld_qwen3_8b_active_10000_hf/checkpoints
...
while true; do
  ...
  for ckpt in "${ckpt_root}"/*/; do
    ...
    for split in eval_in_distribution eval_out_of_distribution; do
      out="${sr_out}/online_q3_${name}_${split}.jsonl"
  ...
  if [ -f "${sr_out}/online_q3_final_eval_out_of_distribution.jsonl.summary.json" ] \
     && [ -f "${sr_out}/online_q3_final_eval_in_distribution.jsonl.summary.json" ]; then
