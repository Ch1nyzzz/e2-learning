#!/usr/bin/env bash
# Non-thinking SR eval for Qwen3-8B base + all checkpoints, GPU 2/3, forward
# order. Companion worker on GPU 0/1 walks in reverse; the progressive jsonl
# guard keeps them from duplicating work.
set -uo pipefail
...
run_sr() {
  ...
  [ -f "${out}" ] && return 0
  echo "=== EVAL-NOTHINK23 ${label} ${split} ==="
