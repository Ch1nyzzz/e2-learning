#!/usr/bin/env bash
# Non-thinking SR eval worker on GPU 0/1: base first (the GPU 2/3 worker is
# still busy with thinking-mode evals for hours, and base-vs-final is the
# decisive nothink comparison), then checkpoints in reverse order so it meets
# the GPU 2/3 worker in the middle.
set -uo pipefail
...
for ckpt in $(ls -d outputs/alfworld_qwen3_8b_active_10000_hf/checkpoints/*/ | sort -r); do
  ...
  for split in eval_out_of_distribution eval_in_distribution; do
    run_sr "online_q3_nothink_${name}" "${split}" --checkpoint "${ckpt%/}"
  done
done
echo "Q3_NOTHINK_EVAL01_DONE"
